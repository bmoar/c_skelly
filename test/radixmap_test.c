...
static char *test_operations() {
    ...
    mu_assert(make_random(map), "Didn't make a random fake map");

    RadixMap_sort(map);
    mu_assert(check_order(map), "Failed to properly sort radixmap");
    ...
    while(map->end > 0) {
        ...
        // test that end is now the old value but uint32 max so it trails off
        mu_assert(check_order(map), "RadixMap didn't stayt sorted after delete");
    }
